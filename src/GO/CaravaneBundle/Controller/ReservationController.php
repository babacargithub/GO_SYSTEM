...
class ReservationController extends MainController{
       ...
       $request=$req;
         ...
         if($resForm->isValid())
        {
          ...
          if($res->getDepart()->getClosRes()==false)  
          {
                ...
                {
                    $errorMsg="Le client a déjé fait une reservation sur ce départ: ".$res->getDepart()->getLibelle();
                }
                else
                { 
                    $em->persist($res);
                    ...
                    $em->flush();
                            ...
                            $depart=$res->getDepart();
                             ...
                             //préparer la notification é envoyer aux clients aprés réservation réuissi
   ...
   /**
    ...
    public function payerAction(Reservation $res, Request $req)
    {
        ...
        $payRepo=$this->getDoctrine()->getRepository('GOCaravaneBundle:Payer');
          $msg=null;
        //========Vérifie si les paiements ne sont pas cléturés pour le départ sélectionné
        if($res->getDepart()->getClosPaye()==false)
        {
            ...
            if(empty($payRepo->findOneByRes($res->getId())))
            {
               ...
               // $em->persist($challengr);
                ...
                $em->flush();
                $res= $this->attribuerNumPlace($res);
                $msg="Paiement enreistré avec succés! Voici le code:  <strong>".$paye->getCodeRecu()."</strong> Numéro place: <strong>".$res->getNumPlace()."</strong> " ;
                $notif="Paiement enreistré avec succés! Numéro place: ".$res->getNumPlace()." Référence : ".$paye->getCodeRecu() ;
                SMSSender::send($res->getClient()->getTel(), $notif);
                }else
                {
                    $this->errorMsg="Impossible d'enregistrer le paiement. Motif: Vous ne disposez pas de compte boutiquier. Seuls les comptes boutiquiers sont autorisés é effectuer des paiements";
                }
            ...
            }else
        {
            ...
            $em->persist($res);
            $em->flush($res);
            $msg="Cette réservation a été déjé payée!";
        }
        }else
        {
    ...
    public function updateAction(Reservation $res, Request $req)
    {
        ...
        if($req->getMethod()=="POST")
        {
                    ...
                    {
                        $msg="Erreur: Les réservations sont cléturées";
                    ...
                    {
                        $notif="Cher client! Suite é la modification de votre réservation, votre numéro de place a changé. Votre avez le numéro ".$res->getNumPlace()." sur le départ: ".$res->getDepart()->getLibelle()."";
                         SMSSender::send($res->getClient()->getTel(), $notif);
                    }
                    $this->msg="Les modifications sont enregsitrées avec succés!";
        return $this->sendResponse(array('view'=>'GOCaravaneBundle::flash_message.html.twig', 'msg'=>$this->msg));
        }else
        {
    ...
    public function confirmAction(Request $req)
    {
       ...
       // $msg="Réservation confirmée!";
        $this->msg="Réservation confirmée avec succés!";
         return $this->sendResponse(array('view'=>'GOCaravaneBundle::flash_message.html.twig', 'msg'=>$this->msg));
    ...
    public function exportAllResEnCoursAction(Request $req)
    {
       ...
       //Instantitiation des classes qui gérenet les exportations selon le format choisi
        ...
        {//déclaration de la variable de base, é laquelle nous ajouterons les objets sous forme de tableau
            //Cet array sera ensuite passé é la fonction qui exporte les données, laquelle fonction prend en parametre
        ...
        ////déclaration de la variable de base, é laquelle nous ajouterons les objets sous forme de text
            //Cette variable sera ensuite passée é la fonction qui exporte les données, laquelle fonction prend en parametre
   ...
   */
    ...
    public function  chiffreDepartAction(Request $req)
    {
        ...
        {
            //Vérifie si l'utilisateur a access é l'application caravane
    ...
    protected function getNotificationMessage(Reservation $res)
    {
        ...
        $salutation="Bnjr"; if(intval(\Date('H'))>=16)$salutation="Bnsoir";
                            ...
                            $depart=$res->getDepart();
                             ...
                             //préparer la notification é envoyer au client aprés réservation réuissi
                             $notification=$salutation."! ".$client_data."! "
                                    . "Réservation é la caravane Golob1 "
    ...
    //exit("you are here without lohin");
      ...
      /* $tableExpoText = new \GO\MainBundle\Utils\TextExporter;
       ...
       //$em->flush();
            ...
            $msg="Le paiement de votre réservation par OM a été enregistré avec succés!";
            $heure_rv=$res->getHeureDepart()->format('H\h:i');
            $num_place=null;
                if($res->isPaye())$num_place=$res->getNumPlace();
                {
                $notification='Aprés paiement par OM, votre numéro de place est le Né'.$num_place.'", RV '.$heure_rv;
